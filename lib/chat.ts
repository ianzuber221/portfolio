...
export function buildSystemPrompt(context: ChatContext): string {
  ...
  return [
    ...
    "- Speak in the third person about Ian. No markdown headings.",
    "",
    `Brief:\n${buildChatKnowledge()}`,
  ]
    .filter(Boolean)
...
export function fallbackReply(
  ...
  if (/passlink|bayer|medical.?device|radiology/.test(question)) {
    return `At Bayer Radiology, Ian led the PassLink Cloud Web App for medical-device software — architecture, delivery, and mentoring offshore developers across complex internal systems. The stack was Angular, Nest.js, AWS Lambda, TypeScript, and PostgreSQL. See [PassLink](/#work) on the work section.`;
  }
  ...
  if (/hire|why|fit|strength|stand out/.test(question)) {
    return `Good fit${forCompany}: Ian ships enterprise Angular front-ends with AI built in (agents, skills, workflows, MCP), led Bayer's [PassLink](/#work) Cloud Web App, and has a public systems story — [Supernova](/work/supernova) at 1,000 req/s under 20ms.${focusNote}`;
  }
